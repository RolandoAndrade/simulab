    ...
    <div class="date-selector">
      ...
      >
        <template v-slot:activator="{ on, attrs }">
          <v-text-field id="ending-time-field"
                        class="caption mx-1"
                        :value="endingDate"
                        :type="isEndADateTime ? 'datetime-local': 'text'"
                        outlined dense hide-details
                        label="Ending time" v-on="on" readonly></v-text-field>
        </template>
            ...
            <v-list>
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-row no-gutters>
                    <v-col cols="12">
                      <v-text-field v-model="endDate" class="caption mx-1" outlined dense hide-details label="Date" type="datetime-local" :disabled="!dateSelected"></v-text-field>
                    </v-col>
                  </v-row>
                </v-list-item-content>
              </v-list-item>
              <v-list-item>
                ...
                <v-list-item-content>
                  <v-row no-gutters>
                    <v-col cols="5">
                      <v-text-field class="caption mx-1" v-model="time.time"
                                    outlined dense hide-details label="Time" type="number"
                                    :disabled="dateSelected" :min="0"></v-text-field>
                    </v-col>
                    <v-col cols="7">
                      <v-select class="caption mx-1" v-model="time.unit" outlined dense hide-details label="Unit" :disabled="dateSelected" :items="[
                      'Seconds',
                      'Minutes',
                      'Hours',
                      'Days'
